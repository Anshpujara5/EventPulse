/**
 * Canonical commerce events grouped by the five tiers in Tracking Contract
 * section 2.
 */
export const COMMERCE_EVENT_NAMES_BY_TIER = {
  tier1CoreFunnel: [
    "product_viewed",
    "add_to_cart",
    "checkout_started",
  ...
  tier5PostPurchase: ["refund_issued", "order_cancelled"],
} as const;

export type CommerceContractTier = keyof typeof COMMERCE_EVENT_NAMES_BY_TIER;
export type CanonicalCommerceEventName =
  (typeof COMMERCE_EVENT_NAMES_BY_TIER)[CommerceContractTier][number];

/**
 * Non-canonical names accepted forever and normalized at read time.
 * New integrations should emit the canonical keys instead.
 */
export const ACCEPTED_EVENT_ALIASES = {
  product_viewed: [
    "product_view",
    "view_product",
...
/** Deprecated event names accepted for backward compatibility. */
export const DEPRECATED_EVENT_NAMES = [
  ...ACCEPTED_EVENT_ALIASES.product_viewed,
  ...ACCEPTED_EVENT_ALIASES.add_to_cart,
  ...ACCEPTED_EVENT_ALIASES.checkout_started,
  ...ACCEPTED_EVENT_ALIASES.purchase_completed,
] as const;

/** Deprecated property forms superseded by envelope identity fields. */
export const DEPRECATED_PROPERTY_NAMES = [
  "customer_id",
  "session_id",
] as const;

/** Future contract event names that merchants must not repurpose. */
...
/**
 * Reserved or forward-compatible property semantics from Tracking Contract
 * sections 3 and 5. The wire keys omit the documentation's [] notation.
 */
