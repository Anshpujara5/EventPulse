...
function allMoneyEvidenceNamesSql(): Prisma.Sql {
  return Prisma.join([
    ...ORDER_FACT_EVENT_NAMES,
...
function salesFactsCtes(eventScope: Prisma.Sql): Prisma.Sql {
  return Prisma.sql`
...
async function fetchSalesHeadline(
  ...
  const rows = await prisma.$queryRaw<HeadlineRow[]>`
    WITH
    ${salesFactsCtes(scope.sql.currentEvent)},
...
async function fetchSalesTrend(input: {
  ...
  const rows = await prisma.$queryRaw<TrendRow[]>`
    WITH
    ${salesFactsCtes(input.scope.sql.currentEvent)},
